...
class SilexPreparedStatement {
	 ...
	 * @param string        $query
	 */
	public function __construct(SilexDatabase $db, PDOStatement $pdoStatement, $query = '') {
		$this->db = $db;
	 ...
	 * @return mixed
	 * @throws SilexDatabaseException
	 * @throws SilexDatabaseException
	 */
	public function __call($name, $arguments) {
		if(!method_exists($this->pdoStatement, $name)) {
			throw new SilexDatabaseException('Unknown method \''.$name.'\'', $this->db, $this);
		}
		try {
			return call_user_func([$this->pdoStatement, $name], $arguments);
		} catch(PDOException $e) {
			throw new SilexDatabaseException('Could not handle prepared statement: '.$e->getMessage(), $this->db, $this);
		}
	}

	/**
	 * @param array $parameters
	 * @return SilexPreparedStatement
	 * @throws SilexDatabaseException
	 */
	public function execute($parameters = []) {
		...
		try {
			...
			throw new SilexDatabaseException('Could not execute prepared statement: '.$e->getMessage(), $this->db, $this);
		}
